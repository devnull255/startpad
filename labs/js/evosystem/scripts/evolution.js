...
Evo.AttrLimits = {
	...
	rOffspring: 1,
	prChangeDir: 1,
	fullthresh: 100,	// Energy field is max 100 - this value subtracted from that and compare to appetite
	};
...
function Evo(options)
{
	this.options = {
		...
		dyWorld: 300,
		StatsUpdateRate: 10,		// Number of Days between refresh
...
Init: function(stWorld, stStats)
{
	...
	this.btnPause.value = "Pause";
	this.btnPause.onclick = this.PauseToggle.FnCallback(this);
    ...
    this.opanel = new OrgPanel(this);
    this.divControl = document.getElementById("Control");
    this.divControl.innerHTML = this.opanel.HTMLForm();
    this.opanel.Bind();
...
CollisionFound: function(org1, org2)
{
    ...
    if(org1.h.preysize >= org2.h.size && org2.fDead == false)
        org1.Eat(org2);
    else if (org2.h.preysize >= org1.h.size && org1.fDead == false)
        org2.Eat(org1);
},
...
function Organism(evo, orgMother, orgFather, energy)
{
	...
	if (orgMother)
		...
		for (var prop in this.h)
			this.h[prop] *= (0.9 + Math.random()*0.2);
...
HTMLForm: function()
{
	var st = "<table class=\"Control\">";
	var stPre = "<tr><th>";
	var stPost = "</td></tr>";
	var cOrg = this.evo.options.StartDensity*(this.evo.mesh.colMax-1)*(this.evo.mesh.rwMax-1)
	st += stPre + "</th><td style=\"text-align:right;\"><input id=\"OP_Create\" type=\"button\" value=\"Create\">" + stPost;
	st += stPre + "Number of Orgs:</th><td><input type=\"text\" id=\"OP_Count\" value=\"" + cOrg + "\">" + stPost;
	for (var prop in this.evo.options.heritable)
		{
		st += stPre + prop + ":</th><td><input type=\"text\" id=\"OP_" + prop +
			 "\"value=\"" + this.evo.options.heritable[prop] + "\">";
		st += stPost;
		}
	st += "</table>";
	return st;
...
Bind: function()
{
	var btn = document.getElementById("OP_Create");
	btn.onclick = this.Create.FnCallback(this);
},
...
Create: function()
{
	...
	inp = document.getElementById("OP_Count");
	num = parseInt(inp.value);
	if (isNaN(num) || num <= 0)
		{
		inp.value = 0;
		alert("Add 1 or more organisms.");
		return;
		}
	...
	for (var prop in this.evo.options.heritable)
		{
		inp = document.getElementById("OP_" + prop);
		num = parseFloat(inp.value);
		if (isNaN(num) || num < 0)
			{
			num = this.evo.options.heritable[prop];
			inp.value = num;
			}
		this.evo.options.heritable[prop] = num;
		}
    for (var i = 0; i < cOrg; i++)
        this.evo.AddOrganism();
        
    if (!this.evo.timer)
		this.evo.PauseToggle();
}
